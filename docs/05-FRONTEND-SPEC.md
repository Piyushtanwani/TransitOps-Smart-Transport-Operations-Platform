...
### `/login`
Split screen: left panel `surface-1` with wordmark "TransitOps" (Barlow SC 700) + one-line tagline "The operations desk for your fleet." Right: card with Email + Password (zod: valid email, min 8), submit "Sign in", inline field errors, top-level error banner for 401 ("Email or password is incorrect."). Below the card, a **Demo accounts** popover listing the four seeded logins (hackathon-judge convenience). On success → role landing: FM/D → `/dashboard`, SO → `/drivers`, FA → `/reports`.
