# 10 — AI Knowledge Base (chatbot static context)

This is the **exact** static context for the chatbot. BE-13 pastes the block below verbatim into `backend/app/services/ai/knowledge.py` as `PROJECT_KNOWLEDGE: str`. At request time, `context.py` appends: (2) the live KPI snapshot, (3) the requester's role + permitted tools statement, (4) the admin's `system_prompt` from `ai_settings`.
...
**Token budget note:** ~520 words ≈ 700 tokens; with live KPIs + role statement + admin prompt the system message stays under ~1,200 tokens — cheap per call at the default model.
